...
ANDROID_NDK_VERSION="$(find "$ANDROID_HOME/ndk" | sort -n | tail -1)"
...
if [ -z ${1+x} ]; then
    printf "%s" "\e[31mDid not pass android build type, using default: ${_DEFAULT_ANDROID_BUILD}\e[30m\n"
	_SHOW_USAGE_INFO=1
else
...
if [ -z ${2+x} ]; then
    printf "%s" "\e[31mDid not pass game name, using default: ${_DEFAULT_GAME_NAME}\e[30m\n"
	_SHOW_USAGE_INFO=1
else
...
if [ -z ${3+x} ]; then
    printf "%s" "\e[31mDid not pass build type, using default: ${_DEFAULT_BUILD_TYPE}\e[30m\n"
	_SHOW_USAGE_INFO=1
else
...
if [ -z ${TW_KEY_NAME+x} ]; then
    printf "%s" "\e[31mDid not pass a key for the jar signer, using default: ${_ANDROID_JAR_KEY_NAME}\e[30m\n"
else
	_ANDROID_JAR_KEY_NAME=$TW_KEY_NAME
fi
if [ -z ${TW_KEY_PW+x} ]; then
    printf "%s" "\e[31mDid not pass a key pw for the jar signer, using default: ${_ANDROID_JAR_KEY_PW}\e[30m\n"
else
	_ANDROID_JAR_KEY_PW=$TW_KEY_PW
fi
if [ -z ${TW_KEY_ALIAS+x} ]; then
    printf "%s" "\e[31mDid not pass a key alias for the jar signer, using default: ${_ANDROID_JAR_KEY_ALIAS}\e[30m\n"
else
...
if [ -z ${TW_VERSION_CODE+x} ]; then
    printf "%s" "\e[31mDid not pass a version code, using default: ${_ANDROID_VERSION_CODE}\e[30m\n"
else
...
if [ -z ${TW_VERSION_NAME+x} ]; then
    printf "%s" "\e[31mDid not pass a version name, using default: ${_ANDROID_VERSION_NAME}\e[30m\n"
else
...
printf "%s" "\e[31mBuilding with setting, for arch: ${_DEFAULT_ANDROID_BUILD}, with build type: ${_DEFAULT_BUILD_TYPE}, with name: ${_DEFAULT_GAME_NAME}\e[1m\n"

if [ $_SHOW_USAGE_INFO == 1 ]; then	
    printf "%s" "\e[31mUsage: ./cmake_android.sh <x86/x86_64/arm/arm64/all> <Game name> <Debug/Release>\e[1m\n"
fi
